...
* If you do any of the file editing on macOS, keep in mind that macOS and Windows both *love* to automatically replace " with “. Tauri/json files care about the difference. So keep an eye on them. You'll see an error like this: ```Error unable to parse JSON Tauri config file at C:\Tauri\gs-tauri\src-tauri\tauri.conf.json because expected value at line 27 column 18```
